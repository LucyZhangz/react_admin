import { Button, message, Steps, Tag, Popconfirm } from 'antd';
import { Modal } from 'antd'
import React, { useState, useRef } from 'react';
import { DownloadOutlined } from '@ant-design/icons';
import { arrang, list, add, dlArrGrade, listByPage, update, listByPagePlan, testdelete } from '../../../api/classManage/taskschedule'
...
export default function ModalBox(props) {
  ...
  const next = async () => {
    if (current == 0) {
      ...
      if (current == 0 && props.newdata) {
        ...
        //获取表格内容
        let inputlist = await listByPagePlan({
          arrId: props.newdata.arrid,
          courseName: '',
          limit: limit,
          page: page,
        })
        inputlist.data.records.forEach(item => {
          item.key = item.id
        });
        console.log(inputlist.data.records);
  ...
  const onEscCan = async (page, limit) => {
    ...
    setpage(page)
    //获取列表内容
    let anfin = await listByPagePlan({
      arrId: props.newdata.arrid,
      courseName: '',
      limit: limit,
      page: page,
  ...
  async function but() {
    // console.log('@@@', delet);
    await testdelete(delet)
    // let anlist = await listByPagePlan({
    //   arrId: props.newdata.arrid,
    //   courseName: '',
    //   limit:10,
    //   page: 1,
    // })
    // anlist.data.records.forEach(item => {
    //   item.key = item.id
    //   console.log(item);
    // });
    // setanfin(anlist.data.records)
    // settotal(anlist.data.total)
    onEscCan(1, 10)
  ...
  const rules = (
    <div style={{ height: '70vh', marginTop: '30px' }}>
      ...
      {/*下面删除 */}
      <br></br>
      <Button type="primary" onClick={but} style={{ width: '100px', marginTop: '10px', marginBottom: '10px' }}>删除</Button>
      <Table
        scroll={{ y: 280 }}
        pagination={false}
        rowSelection={{
